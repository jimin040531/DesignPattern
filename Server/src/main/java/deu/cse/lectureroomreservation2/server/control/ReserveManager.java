    ...
    public class ReserveManager {
        ...
        public static boolean equalsReserveInfo(String fileLine, String requestInfo) {
            try {
                // 1. 파일 라인 파싱 (CSV: 쉼표 기준, 최소 6개 필드 필요)
                String[] fParts = fileLine.split(",", -1);
                if (fParts.length < 6) {
                    return false;
                }

                // 파일의 핵심 키 추출 (건물, 강의실, 날짜, 시작시간)
                String fBuilding = fParts[0].trim(); // 건물 추가
                String fRoom = fParts[1].trim();
                String fDate = fParts[2].trim().replace("-", "/"); // 포맷 통일
                String fStart = fParts[4].trim();

                String cBuilding, cRoom, cDate, cStart;

                // 2. 요청 정보 파싱 (형식에 따라 분기)
                if (requestInfo.contains(",")) {
                    // Case A: CSV 형식 (교수 예약 변경 시 주로 사용)
                    String[] cParts = requestInfo.split(",", -1);
                    if (cParts.length < 6) {
                        return false;
                    }

                    // 요청 정보의 핵심 키 추출 (건물 포함)
                    cBuilding = cParts[0].trim(); // 건물 추가
                    cRoom = cParts[1].trim();
                    cDate = cParts[2].trim().replace("-", "/");
                    cStart = cParts[4].trim();

                    // 3. 핵심 키 비교 (CSV의 경우: 건물, 강의실, 날짜, 시작시간 4가지 모두 비교)
                    return fBuilding.equals(cBuilding) && fRoom.equals(cRoom) && fDate.equals(cDate) && fStart.equals(cStart);

                } else {
                    // Case B: 슬래시 형식 (학생 예약 조회/취소 시 사용)
                    ...
                    // 3. 핵심 키 비교 (슬래시의 경우: 강의실, 날짜, 시작시간 3가지 비교)
                    // 건물 정보가 누락되어 있을 가능성이 높으므로 비교에서 제외
