...
public class ConcreteReservationBuilder implements ReservationBuilder {
    ...
    private void validateReservationDate() {
        String role = reservationDetails.getRole();
        ...
        try {
            ...
            reserveDate = LocalDate.parse(cleanDate, DateTimeFormatter.ofPattern("yyyy/MM/dd"));
        } catch (Exception e) {
        ...
        // [학생(S) 전용 제약] 당일 예약 불가 (내일부터 가능)
        if ("S".equals(role)) {
            if (!reserveDate.isAfter(LocalDate.now())) {
                throw new IllegalArgumentException("학생은 당일 예약이 불가능합니다. 최소 하루 전에 예약해주세요.");
            }
        }
    }
}
